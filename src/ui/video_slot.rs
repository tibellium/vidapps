...
pub struct VideoSlot {
    ...
    /// Index of this slot in the grid (0-3)
    index: usize,
}
...
impl VideoSlot {
    /// Create a new video slot with the given player and index.
    ///
    /// Automatically starts a background task to monitor for video end.
    pub fn new(player: Arc<VideoPlayer>, index: usize, cx: &mut Context<Self>) -> Self {
        let slot = Self { player, index };
